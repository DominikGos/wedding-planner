...
function formatMessage(message: string, t: (key: string) => string) {
  ...
  if (statusChanged) {
    return t('notifications.statusChanged')
      .replace('{{task}}', statusChanged[1])
      .replace('{{from}}', labels[statusChanged[2] as keyof typeof labels])
      .replace('{{to}}', labels[statusChanged[3] as keyof typeof labels])
  }
...
export function NotificationsPanel({ token }: NotificationsPanelProps) {
  ...
  const togglePanel = async () => {
    ...
    try {
      ...
      setError(t('notifications.actionError'))
    } finally {
  ...
  return (
    <div ref={containerRef} className='notifications-panel' style={{ position: 'relative' }}>
      <button type='button' className={unreadCount > 0 ? 'button-primary' : 'button-secondary'} onClick={() => void togglePanel()} style={{ minHeight: '38px', padding: '0.4rem 0.75rem' }}>
        {t('notifications.button')}{unreadCount > 0 ? ` (${unreadCount})` : ''}
      </button>

      {open && (
        <section className='page-card notifications-dropdown' style={{ position: 'absolute', top: 'calc(100% + 0.6rem)', right: 0, width: 'min(380px, calc(100vw - 2rem))', padding: '1rem', display: 'grid', gap: '0.75rem', zIndex: 1200 }}>
          <strong>{t('notifications.title')}</strong>
          {!token && <span style={{ color: 'var(--muted)' }}>{t('notifications.requiresLogin')}</span>}
          {token && loading && <span style={{ color: 'var(--muted)' }}>{t('notifications.loading')}</span>}
          {error && <span style={{ color: '#c53030' }}>{error}</span>}
          {token && !loading && !error && notifications.length === 0 && <span style={{ color: 'var(--muted)' }}>{t('notifications.empty')}</span>}
            ...
            <div style={{ maxHeight: '330px', overflowY: 'auto', display: 'grid', gap: '0.75rem', paddingRight: '0.25rem' }}>
              {notifications.map(notification => (
                <div key={notification.id} style={{ padding: '0.75rem', borderRadius: '10px', border: '1px solid var(--border)', background: unreadBeforeOpen.includes(notification.id) ? 'var(--primary-soft)' : 'var(--surface)', display: 'grid', gap: '0.45rem' }}>
                  <span style={{ whiteSpace: 'normal', lineHeight: 1.4 }}>{formatMessage(notification.message, t)}</span>
                  <small style={{ color: 'var(--muted)' }}>{new Date(notification.createdAt).toLocaleString(i18n.language === 'pl' ? 'pl-PL' : 'en-US')}</small>
                </div>
              ))}
            </div>
          )}
